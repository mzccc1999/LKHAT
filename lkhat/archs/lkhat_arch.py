...
class DilatedReparamBlock(nn.Module):
    ...
    def merge_bn(self):
        if hasattr(self, 'origin_bn'):
            ...
            # self.lk_origin = nn.Conv2d(channels, channels, kernel_size, stride=1, padding=kernel_size // 2, dilation=1, groups=channels, bias=True)
            self.lk_origin.weight.data = origin_k
            self.lk_origin.bias = torch.nn.Parameter(torch.ones_like(origin_b), requires_grad=True)
            ...
            for k, r in zip(self.kernel_sizes, self.dilates):
                ...
                conv.weight.data = branch_k
                conv.bias = torch.nn.Parameter(torch.ones_like(branch_b), requires_grad=True)
                conv.bias.data = branch_b
                self.__delattr__('dil_bn_k{}_{}'.format(k, r)) 
                                         

    def merge_dilated_branches(self):
        origin_k, origin_b = self.lk_origin.weight, self.lk_origin.bias
        for k, r in zip(self.kernel_sizes, self.dilates):
            conv = self.__getattr__('dil_conv_k{}_{}'.format(k, r))
            branch_k, branch_b = conv.weight, conv.bias
...
class HACB(nn.Module):
    ...
    def __init__(self,
        ...
        self.conv_block = UniRepLKNetBlock(dim, rep_kernel_size, deploy=deploy, ffn_factor=compress_ratio)
    ...
    def __init__(self,
        ...
        self.blocks = nn.ModuleList([
            HACB(
                ...
                rep_kernel_size=rep_kernel_size,
                ...
                deploy=deploy) for i in range(depth)
        ])
